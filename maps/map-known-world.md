# THE KNOWN WORLD
## Regional Map of the Hearthcoast and Interior

> _"Ink on vellum, copied from port ledgers, Archive road-maps, and marshwarden testimony."_

```text
  WESTERN COAST                       FAR NORTH                        NE COAST
       │                                 ↑                                │
       │                    ┌────────────────────────┐                    │
       │                    │  ⚒ WARDEN'S FORGE      │                    │
       │                    │  🕳 U7 (sealed tunnels) │                    │
       │                    └───────────┬────────────┘                    │
       │                                │                                 │
       │                        Vigil Stone Road                          │
       │                          (2 days south)                          │
       │                                │                                 │
       │                    ╔═══════════════════════╗                     │
       │                    ║   GREYVEIL PASS       ║                     │
  ⊗ Node 5 (74%) ✔         ║  ⊗ Node 12 (72%) ✔   ║                     │
  (Stormcape)               ╚═══════════╤═══════════╝                    │
  RECONSTRUCTED                         │                                │
       │          ⊗ Node 11 (75%) ✔     │        ⊗ Node 10 (80%) ✔      │
       │         (Sethrak's Threshold)  │       (Cape Ashward)           │
       │            RECONSTRUCTED       │                                │
       │                                │                                │
  ⊗ Node 3 (62%) ✔        The Greypeak Mountains                        │
  (Storm Pillar)          ~~~~~~ 🕳 U4 ~~~~~~                            │
  REPAIRED                /  (pass caverns)   \                          │
       │                 /                      \                         │
       │  ┌──────────┐ /                         \ ┌─────────────┐       │
       │  │ GREYMOOR │/                            │  AELINDRA   │       │
       │  │ (mining) │                             │ (cliff city) │       │
       │  │🕳U2 Shaft 9                            │🕳U10 (vaults)│       │
       │  └────┬─────┘                             └──────┬──────┘       │
       │       │       ░░ THORNVEIL ░░                    │              │
       │   Iron Road   ░░ 🕳 U5 (sinkholes) ░░    Archivists' Road      │
       │       │       ░░░░░░░░░░░░░░░░░░░░░░░            │              │
       │       │  ⛪                    │           ┌──────┴──────┐       │
       │       │ Ashenmere       🕳 U9 (Mirren)    │ ⊗ VEL THARIS│       │
       │       │ 🕳U3 (lower vault)    │           │ Node 9 (78%)│       │
       │       │                ┌──────┴──┐        │ ✔ 🔒 SEALED │       │
       │       │                │FENHOLLOW│        └──────┬──────┘       │
       │       │                │(marshes)│               │              │
       │       │                └┬──┬──┬──┘        ┌──────┴──────┐       │
       │       │        ⌂ Dawnwatch │  │           │ SALTHOLLOW  │       │
       │       │        Homestead   │  │     ┌─────┤(university) │       │
       │       │      ★ PARTY HERE  │  │     │     └─────────────┘       │
       │       │        ☒ Barrowfen │  │     │                           │
  ⊗ Node 4    │        (🔒 SEALED) │  │  ┌──┴──────┐   Scholars'        │
  (Tidewatcher)│   🕳 U6 (Sealed   │  │  │HARROWGATE│    Coastal         │
  76% ✔        │    Ruins, marshes) │  │  │ (trade)  │     Road           │
  INFUSED      │                │   │  │  └──┬──────┘      │             │
       │       │       🏨 Thornfield│  │     │              │             │
       │       │          Dispatch  │  │  King's Road       │             │
       │       │                │   │  │     │              │             │
       │       │          Fenhollow Marshes   │              │             │
       │       │           ≈≈≈≈≈≈≈  │  │     │              │             │
       │       │               │    │  │     │              │             │
       │       │               │ ⚓ TIDEMEET │              │             │
       │       │               │ (river mouth)              │             │
       │       │               │    │  │     │              │             │
       │       └───────────────┴────┴──┴──┬──┴──────────────┘             │
       │                                  │                               │
  ⊗ Node 2 (71%) ✔                  ┌────┴─────┐                         │
  (Drowned Forge)                   │ DUSKPORT │                         │
  Ashfire Isle                      │ (harbor) │                         │
  REPAIRED                          │🕳U1 (Undercroft)                    │
       │                            └────┬─────┘                         │
       │                                 │                               │
       │                     ════════════╪═══════════════                 │
       │                       The Hearthcoast Sea                       │
       │                     ════════════╪═══════════════                 │
       │                                 │                               │
       │                          ┌──────┴───────┐                       │
       │                          │PORT VELDMERE │                       │
       │                          │(southern port)│                       │
       │                          └──────┬───────┘                       │
       │                                 │                               │
       │                        Southern Sea Routes                      │
       │                           (open water)                          │
       │                                 │                               │
       │                     🕳 U11 (volcanic island,                     │
       │                      Node 1 area, far south)                    │
       │                                                                 │
  STORMCAPE ALLIANCE                                                     │
  (5 cliff-holds: Arn,                              TRADE ROUTE          │
   Kell, Venn, Driftwood,              Tidemeet ←→ Veldmere ←→ Stormcape│
   Saltspray + Asha)                   *Stormcape's Promise* (trimaran)  │
```

### Map Legend
| Symbol | Meaning |
|--------|---------|
| ⊗ | Outer Ward Node |
| ✖ | Failed Ward Node |
| ✔ | Repaired Ward Node |
| 🔒 | Sealed Barrow |
| ⌂ | Dawnwatch Homestead |
| ⛪ | Ruined Temple (Ashenmere) |
| ⚒ | Warden's Forge (Vigil) |
| 🏨 | Thornfield Dispatch (coaching inn) |
| ⚓ | Tidemeet (river mouth / port) |
| ☒ | Investigated site |
| ☆ | Mountain pass |
| ★ | Current party location |
| 🕳 | Suspected/Confirmed Underdark Entrance |

---

## CITIES

### Duskport
Mid-coast harbor city. Seat of Magistrate Voss's inquiry. Home to the Temple of the Balanced Scale, harbor district, Scales District, Ashward (working-class), and council chambers on the hill.

- Population: ~12,000
- From Homestead: 3 days overland (via Fenhollow → Harrowgate → King's Road), or 2 hrs to Tidemeet + 2 days by ship

### Aelindra
Northern cliff city and home of the Grand Archive. Seat of interim Archivist-General Veyra Solund. Built into a cliff face in four tiers from harbor to Crown. Politically sovereign. 🕳 **Suspected Underdark entrance (U10):** Deep vaults below the Archive — unknown depth. Thessara decoded 6 sealed site locations. Sub-level 2 was deepest explored.

- Population: ~8,000 (mostly scholars and Archive staff)
- From Homestead: 3.5 days by charter carriage, or 2 hrs to Tidemeet + 3–4 days by ship

### Salthollow
Northeastern coastal university town. Home of the Athenaeum (Lorekeeper Idris Vane's private archive). Smaller than Duskport; no walls, no garrison, centered around the university.

- Population: ~5,000
- From Homestead: 4 days overland (via Harrowgate → Scholars' Coastal Road), or via Tidemeet by ship (~3 days)

### Harrowgate
Inland trade city at the crossroads of the King's Road and the Mirren River. Seat of the Coinwright's Guild (Guildmaster Orvyn Tetch). Largest market in the region.

- Population: ~15,000
- From Homestead: 2 days overland (south through marshes to King's Road)

### Greymoor
Northern mining city in the Greypeak Mountains. Seat of the Pallid Court. Known for iron, silver, and deep-shaft mining. Cold, industrial, isolated. 🕳 **Confirmed Underdark entrance (U2):** Shaft 9 — sealed door at 600 ft depth (found by Donal Crewe). Miners report "singing from the ground" (Durnock).

- Population: ~7,000
- From Homestead: 5 days by charter carriage (Iron Road), or 2 days south to Fenhollow + 3 days north via Thornveil Forest

### Fenhollow
Northwestern marshland settlement and base of the Thornwatch (Captain Jorik Brenn, reinstated). Not a true city, but raised villages linked by boardwalks and causeways through the marshes.

- Population: ~3,000 (distributed)
- From Homestead: 20 minutes walk / immediate vicinity

### Port Veldmere
Southern port city across the Hearthcoast Sea from Duskport. Seat of Marchion Drael Sulwick's former trade empire (dismantled). Controls southern sea routes.

- Population: ~18,000
- From Homestead: 1.5 days via Tidemeet river route, or 3 days overland to Duskport + 3 days by ship

### Ashport
Southern fishing and timber port. Warm climate. Independent — no civic alliance. Friendly to traders. Tropical hardwood, citrus, spices, fish oil.

- Population: ~6,000
- From Homestead: 4 days sail south of Veldmere (~7 days total from Tidemeet)

### Coral Haven
Island port on a reef in the southern waters. Pearl divers. Isolated, suspicious of outsiders. Revka has one contact.

- Population: ~3,000
- From Homestead: 7 days sail south of Veldmere (~10 days total from Tidemeet)

### Sunmere
Major southern city. Warm, wealthy. Primary surface-Underdark trade nexus — Drow merchants (including Vel'sharen agents) surface here. Silk, dyes, exotic spices, adamantine, gems. Nhilara's former base of operations.

- Population: ~10,000+
- From Homestead: 10 days sail south of Veldmere (~13 days total from Tidemeet)

---

## SEAS & WATERWAYS

### The Hearthcoast Sea
The central body of water connecting Duskport, Veldmere, Salthollow, and Aelindra. Busy shipping lanes; Sulwick's fleet impounded, Silver Chain now patrols southern routes.

### The Mirren River
Flows from Greypeak foothills southeast through Harrowgate and toward the coast east of Duskport. Primary inland trade artery; Tetch's guild frozen, under civic oversight.

### The Fenhollow Marshes
Vast wetlands northwest of Harrowgate. Tidal and treacherous; navigable mainly by locals and Thornwatch guides. Three sealed pre-Sundering ruins appear at low tide.

---

## FORESTS & WILDERNESS

### Thornveil Forest
Dense old-growth forest between Greymoor and Fenhollow. The Iron Road skirts its eastern edge. Interior is largely unmapped; locals avoid it. Orindel hunts along the southern edge near the homestead. 🕳 **Suspected Underdark entrance (U5):** Two sinkholes scouted by Orindel + Seren (Day 170–171). Large sinkhole (~250 ft deep, limestone collapse). Small sinkhole (warm air rising — geothermal or large enclosed space). Both marked on Torven's map.

### The Greypeak Mountains
Northern mountain range. Greymoor sits in a southern valley. Greypeak Pass is the only reliable crossing to lands beyond. The Greypeak Reavers (10–15 bandits) operate in the pass — opportunists, avoid large armed groups. Griffons nest in the high crags (territorial during Highsun breeding season). 🕳 **Suspected Underdark entrance (U4):** Limestone karst geology — riddled with natural caverns. Vaelith: "The mountains are hollow. They always were."

### The Northern Highlands
Barren scrubland and exposed rock beyond Greypeak Pass. No settlements. No trees. Constant wind. The Vigil Stone Road runs north along the ridge. Temperature drops sharply. Animal life thins as you move north — the ward boundary's influence. Ogres reported in the highland scrubland (2–3, territorial). Possible troll in a ravine (unconfirmed). At night, shapes are visible on the eastern ridgeline — entities on the other side of the thinning ward boundary. **Post-Node 12 repair:** entities withdrew from Greyveil Pass ridgelines. Ward dome visible and stable at 72%.

### The Dead Zones (Beyond the Outer Wards)
Lands beyond the outer ward perimeter. Pre-Sundering texts describe them as "lands where death is ambient" — necrotic energy saturates the environment. Prolonged exposure causes exhaustion, necrotic damage, then death. The wards don't just keep things out — they keep the death energy from spreading inward. **Post-repair status:** With all 8 known outer nodes restored and the cascade ended, the ward boundary has stabilized. Dead zones are actively recovering — the necrotic field at Sethrak's Threshold evaporated upon reconstruction, and the Stormcape Dead Zone is clearing after Node 5 reconstruction. Enchanted Dawnwatch rings provide ward-network recognition but not full environmental protection beyond the ward line.

---

## ROADS & ROUTES

### King's Road
Main overland artery from Duskport north through Harrowgate and onward toward Greymoor.

### Iron Road
Branches from King's Road at Harrowgate and climbs into the Greypeaks toward Greymoor. Rough, steep, wagon-accessible, and slow; ore caravans use it constantly.

### Thornfield Road
Runs north from Fenhollow through Barrowfen and into the northern countryside. Unpaved but well-traveled. Thornfield Dispatch coaching inn located along this road. Dead drop sites and the route to Keth-Varos branch off from here.

### Archivists' Road
Coastal route connecting Aelindra to Salthollow. 2 days overland. Well-maintained by Archive funding.

### Scholars' Coastal Road
Connects Salthollow south to Duskport along the northeastern coast. 3 days overland. Less maintained. Fishing villages along route.

### Vigil Stone Road (Northern Highlands)
Ancient pre-Sundering road running north from Greypeak Pass along the highland ridge toward Greyveil Pass and beyond. Fitted stone, no mortar — Vigil of the Closed Eye construction. Still intact after 1,200+ years. No settlements. Vigil wardens (undead sentinels) still stand guard at intervals. The road is a ward conduit — residual energy keeps the stones warm and the wardens animated. Discovered by the Dawnwatch during the Greyveil expedition. **Extends north past Greyveil Pass to the Warden's Forge (2 days) and branches toward Sethrak's Threshold (Node 11).**

### Sea Routes (from Tidemeet)

| From | To | Time |
|---|---|---|
| Tidemeet | Aelindra | 3–4 days north |
| Tidemeet | Duskport | 2 days south |
| Tidemeet | Salthollow | 2.5 days northeast |
| Duskport | Port Veldmere | 3 days south |
| Salthollow | Aelindra | 1.5 days (coastal hop) |
| Veldmere | Ashport | 4 days south |
| Veldmere | Coral Haven | 7 days south |
| Veldmere | Sunmere | 10 days south |

---

## DISTANCE TABLE — From Dawnwatch Homestead

| Destination | Overland | By Sea (via Tidemeet) | Notes |
|---|---|---|---|
| **Fenhollow** | 20 min walk | — | Immediate vicinity |
| **Tidemeet** | 2 hrs by cart | — | River mouth, coastal port |
| **Barrowfen** | Half-day north (Thornfield Road) | — | |
| **Keth-Varos** | Half-day + 1 hr NE of Barrowfen | — | Sealed barrow |
| **Thornfield Dispatch** | Half-day north (Thornfield Road) | — | Coaching inn |
| **Harrowgate** | 2 days south | — | Via marshes to King's Road |
| **Duskport** | 3 days overland | 2 days from Tidemeet | Via Harrowgate |
| **Greymoor** | 5 days (charter carriage, Iron Road) | — | Via Harrowgate |
| **Ashenmere** | 5 days + 3 days walk from Greymoor | — | Ruined temple, hills east of Greymoor |
| **Aelindra** | 3.5 days (charter carriage) | 3–4 days from Tidemeet | |
| **Vel Tharis** | 3.5 days to Aelindra + half-day north | — | Coastal cliffs, sealed barrow |
| **Salthollow** | 4 days overland | 2.5 days from Tidemeet | Via Harrowgate + Scholars' Road |
| **Port Veldmere** | 1.5 days (Tidemeet river route) | 5 days (Tidemeet → Duskport → Veldmere) | River route faster |
| **Greypeak Pass** | 5 days to Greymoor + 3 days climb | — | Mountain crossing |
| **Greyveil Pass (Node 12)** | ~13 days total | — | Greymoor (5d) → Greypeak Pass (3d) → Vigil Road (4–5d) |
| **Warden's Forge** | ~15 days total | — | Greyveil Pass + 2 days north on Vigil Road |
| **Sethrak's Threshold (Node 11)** | ~15 days total | — | Warden's Forge + 2 days (or Greyveil + branch NE) |
| **Cape Ashward (Node 10)** | — | ~5–6 days from Tidemeet (via Aelindra + 2 days ship) | NE coastal headland. Ashward's Landing fishing village. |

---

## LANDMARKS & NOTABLE LOCATIONS

### Dawnwatch Homestead (Fenhollow) ⌂
Four-acre homestead on an inland rise, 20 min from Fenhollow village. Stone perimeter wall (6 ft, limestone, completed Day 68 1249 AS) with main gate (south, oak, iron-banded, ward-locked) and postern gate (north, marsh access, ward-locked). Ward perimeter (invisible): extended 500 ft detection, Drow-specific signature, silent alarm, tower-linked.

**Structures:** Main house (2 stories, 4 bedrooms + Tessa's loft, moonpetal display case), Dawnwatch Tower (4 levels + lookout ring, telescope, ward-glyph alarm, library L4), alchemy shed (Vaelith's loft), forge (+ Ashara containment room, lead-lined, ward-suppressed), smokehouse, tannery/leatherworking shed, stable (3 stalls, hay loft), root cellar, mushroom cellar (exotic — 3 Underdark species producing, bioluminescent), school (operational — Elara Marsh + Dael Ashwick + Brother Callum, 15 students, Vel'thara curriculum), goat pen (2 goats), chicken coop, beehives (3 + second rack).

**Gardens:** Herb garden (doubled, ghost-nettle shade structure), Mira's garden (ornamental + vegetable + medicinal, moonpetal seeds planted).

**Library:** 29 books (14 acquired + 6 authored + 8 ordered + Atlas of the Vel'thara).
**Residents:** 10 Dawnwatch + Vaelith + Tessa + Ren (12) + Lila (9) + Ashara (prisoner, containment) + Edrin Vael (prisoner) + Thessara Vael (probation) + Drael + Syvra (prisoners).
**Passive income:** 35–45 gp/week. Apothecary contract: Master Haldren. Delivery: Wynn Dodd.
**Defenses:** Stone wall, ward perimeter (4 layers), tower fallback, Deep Road evacuation route.
**Roads:** All-weather gravel road to Fenhollow (10 min by carriage). Raised causeway to Tidemeet (1 hr by carriage). Carriage: *Dawnwatch Coach* (resonance-assisted, ward-dampened, 2 horses).

### Vel'quinar Homestead (Fenhollow) — COMPLETE
Three-acre parcel north of the Dawnwatch Homestead. Settlement writ issued by Captain Brenn (Day 131, 1249 AS). Sponsored by the Dawnwatch. Home to 16 members of House Vel'quinar.

**Structures:** Main house (single story, 6 rooms, shade-designed for Drow), underground quarters (5 rooms + common room, bioluminescent lichen lighting), underground armory (ward-locked), 2 prison cells (ward-suppressed), forge/workshop (shared with Dawnwatch), shade garden (Nalia's apothecary).

**Residents (16):** Matron Ilyana Vel'quinar, Kael (Weapons Master), Sera (Priestess of Eilistraee), Nalia (healer), Dren (smith — crewing *Eilistraee's Grace*), Vel (scout), Thorn + Vex (warriors/sentries), Ryn (apprentice wizard), Pim (14, student), Ash (elder historian), Lyriel (captain — crewing *Eilistraee's Grace*), Nhilara (Dawnwatch specialist), Velyndra Vel'nessra (glassworker, defected S8), Taelar Vel'nessra (weaver, defected S8), Sylla Vel'nessra (age 7, student #15, defected S8).

**Defenses:** Stone wall. Ward perimeter (Drow-specific, tower-linked). Underground armory (4 hand crossbows, 1 heavy crossbow, 4 dark steel spears, 4 reinforced shields, 5 leather sets). 8 combat-capable. 3-shift night watch. Underground fallback position.
**Income:** ~75 gp/month (rising). Loan: 175 gp at 15 gp/month.
**New trades:** Spider-silk weaving (Taelar + Thessaly), Drow glasswork (Velyndra + Sylvara).

### Tidemeet ⚓
River mouth where the Fenhollow river meets the Hearthcoast Sea. Sandy beach on the south bank, rocky tide pools on the north. Half-day walk south of Fenhollow (2 hours by cart). Coastal vessels dock here — sea route to Aelindra (3–4 days).

### Thornfield Dispatch 🏨
Coaching inn and courier service on the Thornfield Road, north of Fenhollow. Operated by Harlen Grubb. Used as the logistics channel for the Watcher's necromantic operation — dead drops, payments, and correspondence. Grubb cooperated with the Dawnwatch. The courier route is now compromised.

### Temple of Quiet Passage / Ashenmere ⛪
Ruined temple of the Order of the Ashen Veil. Located in Ashenmere Valley, hills east of Greymoor (3 days' walk). Destroyed by Veradis Kael's necromantic attack — 17 members of the Order killed. Site sealed by Greymoor patrol. A sealed vault in the lower catacombs remains unopened and unexplored. **Vigil Heartstone (1/7) discovered — active. Greymoor garrison posted (Commander Cole).** 🕳 **Suspected Underdark entrance (U3):** Lower vault extends below known levels. The Order built above something older.

### Barrowfen ☒
Small hamlet on the Thornfield Road, half-day north of Fenhollow. Site of Veradis Kael's catalyst field tests — 5 graves disturbed, 3 zombies raised and subsequently destroyed by the Dawnwatch. Village cemetery now quiet.

### Barrow of Keth-Varos 🔒
Pre-settlement ossuary barrow on a treeless hilltop northeast of Barrowfen. ~300 ancient remains. Ringed by standing stones. Catalyst was never applied — barrow confirmed inert. Entrance collapsed and sealed by the Dawnwatch with Arcane Lock. First barrow secured.

### Barrow of Vel Tharis 🔒
Pre-settlement barrow complex on coastal cliffs north of Aelindra. ~400–500 ancient remains. Sea cave entrance (accessible at low tide only, 6-hour window). Standing stone with closed eye symbol on the cliff top near Node 9. Second operative intercepted here before catalyst application. Entrance collapsed and sealed by the Dawnwatch with Arcane Lock. Second barrow secured.

### Greyveil Pass ✔
Northern mountain pass beyond the Greypeaks. Location of outer ward Node 12 — **REPAIRED by the Dawnwatch (28% → 72%)**. Site of the meeting with Vaelith, Last Warden of the Vigil. Warden's Vault (emptied — resonance crystal installed, field manual retrieved). Three undead Vigil wardens stand guard on the stone road. Ward dome now visible and stable. Entities withdrew from ridgelines after repair.

### Cape Ashward ✔
Northeastern coastal headland. Location of outer ward Node 10 — **REPAIRED by the Dawnwatch (31% → 80%)**. Standing stone on the highest point, singing again for the first time in decades. Fishing village of Ashward's Landing in the sheltered cove below (~200 people, headman Corran Ashward). Salt-ward beeswax coating applied to copper circuit for coastal protection. *Tern's Wing* at anchor in the cove.

### Warden's Forge ⚒
Vigil of the Closed Eye's primary workshop, located in a mountain hollow 2 days north of Greyveil Pass on the Vigil Stone Road. Stone building built into mountainside. Self-sustaining forge-ward has burned for 1,000+ years. Contains: banked forge (operational), crystal-cutting tools, ward-imprinting basin, raw resonance stone (2.0 blocks remaining). The Dawnwatch cut resonance crystals for Nodes 5 and 11, enchanted all 10 Dawnwatch rings, and enchanted all 8 armor sets (+1 AC) here. Sethrak's Forge-Master Tools discovered (Bram). **Beyond the ward line — no ward dome protection.** 🕳 **Suspected Underdark entrance (U7):** Tunnels extend below forge level — sealed by the Vigil 1,000+ years ago. Vaelith: "We sealed them because things came up."

### Sethrak's Threshold (Node 11) ✔
Location of outer ward Node 11 — former cascade origin point. **RECONSTRUCTED by the Dawnwatch** (FAILED → 75%). Full 6-anchor reconstruction with Zaryth as Warden's Anchor. 14 ward-wraiths (inverted Vigil wardens) communicated with and freed peacefully — zero combat. Golden light column tore the sky open. Named for Captain Sethrak, Vaelith's commanding officer. ~2 days travel from the Warden's Forge. Territory recovering — necrotic field evaporated upon repair.

### Greypeak Pass ☆
Only reliable mountain crossing. Controlled by Greymoor; strategic chokepoint.

### Stormcape (Node 5) ✔
Western coastal cliff, Seren Ashvale's former post (31 years). **RECONSTRUCTED by the Dawnwatch** (FAILED → 74%). 6-anchor reconstruction in the Dead Zone. Dead Zone clearing. 5 cliff-holds allied (Arn, Kell, Venn, Driftwood Landing, Saltspray). Pigeon network to homestead.

### Storm Pillar (Node 3) ✔
Western headland, Stormcape coast. **REPAIRED by the Dawnwatch** (62%). Field-calibrated crystal. Behir nest cleared (Young Adult CR 6 + 2 Juvenile CR 3 — hardest fight of the campaign).

### Drowned Forge (Node 2) ✔
Volcanic sea cave, Ashfire Isle. **REPAIRED by the Dawnwatch** (71%). Warden Drell found deceased at post (38 years service). Pre-tuned crystal from her cache. 🕳 **Suspected Underdark entrance (U8):** Bioluminescent fungus suggests deep ecosystem. Drell's journal mentions "sounds from below the third chamber."

### Tidewatcher (Node 4) ✔
Tidal rock shelf, western coast. **INFUSED by the Dawnwatch** (48% → 76%). Accessible ~3 days/year. Pip's first flawless procedure. Folk ward-keeper Asha (age ~12, Grandmother Wynn's granddaughter) maintained it with shell offerings for years.

### The Undercroft (Duskport) 🕳
Pre-Sundering underground complex beneath the city. Repository opened by the party in Session 1. Hollow Court origin. Now known to Voss's inquiry. **Confirmed Underdark entrance (U1).** Shallow depth (50–200 ft). Deeper levels unexplored. Possible connections to deeper Underdark systems.

### The Three Sealed Ruins (Fenhollow Marshes) 🕳
Pre-Sundering stone structures visible at low tide. **SCOUTED by Dawnwatch (Day 170).** Not buildings — they are *caps* over a subterranean structure. Torven (druid sense, DC 22): stone extends 40+ ft below surface. Mira (Investigation 20): sealed door found — precision-cut, hidden mechanism. Ward compass detected Binding frequency (pre-ward, older than the 14-node network). Vaelith confirms: "What the Binders sealed, they sealed for a reason." **A fourth ruin exists — fully submerged, deeper in the marsh. Old Harren Fenn: "That one is the real door. The other three are just the porch."** Thornwatch blocks access by ancestral tradition. Closest potential Underdark entrance to the homestead (half-day). **Priority 1 for Underdark expedition.**

### Binding-Era Keeper Station (Fenhollow Marshes) 🕳
Discovered Day 185 beneath the Fourth Ruin. Circular chamber, 60 ft across, domed ceiling. Resonance anchor = Node 7 (91%, self-maintaining, direct Root connection). Vaelen's journal recovered. Ward instruments. Pure resonance crystal. Carved map of all 14 nodes. 8 focus stone pedestals (Heartstones). Deeper door — palm recognition lock (Zaryth attuned Day 216). Below: the Deep Road — natural cavern system descending toward the Greypeaks. Fungal Gallery (safe rest point). The Listening Dark (vast void). The Old Foundation at 540 ft depth — pre-Binder platform, source of the Binding frequency. Unknown script. Zaryth's signet ring reacted to central glyph.

### The Grand Archive (Aelindra)
Four tiers. The Crown summit holds restricted vaults. Sub-level 2 (Thessara's chamber) secured. The Sealed Quorum dissolved. Sylvara Dawnwhisper holds permanent Level 3 restricted access. Interim Archivist-General: Veyra Solund. Thessara Kain under house arrest.

### The Athenaeum (Salthollow)
Pre-Sundering sealed vault beneath the university quarter. Contains original civic charters for every town in the river basin. Restored to civic control after Dawnwatch operation.

### The Coinwright's Guildhall (Harrowgate)
Tetch's former seat of power. Guild frozen under civic oversight. Corvath arrested. Clearinghouse ledger seized.

---

## OUTER WARD NETWORK

The continental ward system has two layers:

**Inner Ward (The Root):** Renewed by the Dawnwatch (Session 4). Active. Healthy. Protects the heartland.

**Outer Ward:** 14 anchor nodes forming a perimeter around the continent. Pre-Sundering construction. Maintained by the Vigil of the Closed Eye until the Sundering. **ALL 8 KNOWN OUTER NODES REPAIRED by the Dawnwatch. Cascade ended. Network stable and self-reinforcing. First time in 1,248 years.**

### Known Nodes (Repaired — ALL 8 ACTIVE)

| Node | Location | Status | Notes |
|------|----------|--------|-------|
| **Node 2** | Drowned Forge, Ashfire Isle (volcanic sea cave, B12) | **71% ✔ REPAIRED** | Pre-tuned crystal from Warden Drell's cache. Drell found deceased at post (38 years service). |
| **Node 3** | Storm Pillar, western headland (Stormcape, A7) | **62% ✔ REPAIRED** | Field-calibrated crystal. Behir nest cleared (mother CR 6 + 2 juveniles CR 3). Day 27, 1248 AS. |
| **Node 4** | Tidewatcher, tidal rock shelf (A10) | **76% ✔ INFUSED** | Resonance infusion (maintained circuit). Pip's first flawless procedure. Folk ward-keeper Asha (Grandmother Wynn's granddaughter) maintained it with shell offerings. Accessible ~3 days/year. |
| **Node 5** | Stormcape, western coast (A5) | **74% ✔ RECONSTRUCTED** | 6-anchor reconstruction in Dead Zone. Forge-cut crystal. Seren Ashvale's former post (31 years). Dead Zone clearing. |
| **Node 9** | Vel Tharis, coastal cliffs N of Aelindra (O4) | **78% ✔ REPAIRED** | Standing stone singing. Cache retrieved. Barrow sealed below. |
| **Node 10** | Cape Ashward, NE headland (Q3) | **80% ✔ REPAIRED** | Highest reading. Standing stone singing. Ashward's Landing fishing village. |
| **Node 11** | Sethrak's Threshold, northern wastes (D1) | **75% ✔ RECONSTRUCTED** | Cascade origin. 6-anchor, Zaryth as Warden's Anchor. 14 ward-wraiths freed peacefully. Zero combat. |
| **Node 12** | Greyveil Pass (E2) | **72% ✔ REPAIRED** | Vaelith's vigil site. Ward dome stable. Entities withdrew voluntarily. |

**ALL 14 OUTER WARD NODES ACTIVE — first time in 3,248 years.**

Ward network: Root (active) + 14 outer nodes + Anchor Point (permanent hub).

| Node | Name | Location | Reading | Method |
|------|------|----------|---------|--------|
| 1 | Southern Deep | Volcanic island S of Veldmere (L16) | **64%** | Remote boost from Anchor Point |
| 2 | Drowned Forge | Ashfire Isle (B12) | **71%** | Physical repair |
| 3 | Storm Pillar | Western headland (A7) | **62%** | Physical repair + behir combat |
| 4 | Tidewatcher | Tidal shelf (A10) | **76%** | Physical infusion |
| 5 | Stormcape | Western coast (A5) | **74%** | Physical reconstruction |
| 6 | Thornveil Heart | Deep Thornveil Forest (F7) | **69%** | Remote boost from Anchor Point |
| 7 | Marsh Threshold | Fenhollow keeper station (G8) | **91%** | Self-maintaining |
| 8 | River Memory | Upper Mirren River (G7) | **71%** | Remote boost from Anchor Point |
| 9 | Vel Tharis | Coastal cliffs N of Aelindra (O4) | **78%** | Physical repair |
| 10 | Cape Ashward | NE headland (Q3) | **80%** | Physical repair |
| 11 | Sethrak's Threshold | Northern wastes (D1) | **75%** | Physical reconstruction |
| 12 | Greyveil Pass | (E2) | **72%** | Physical repair |
| 13 | Sunken Crown | Submerged reef, 40 mi E of Cape Ashward | **67%** | Underwater physical reconstruction (S8) |
| 14 | Ember Vein | Beneath Greypeaks (E4) | **68%** | Physical repair via Ashara station (S7) |

### Anchor Point (Pre-Binder Prototype)
400 ft beneath Thornback Gulch, Ashport. Control interface for the entire network. **PERMANENTLY ACTIVE** — 7 Heartstones seated. Vaelen + Warden Drell stationed as keepers. **Vel'thara embassy** — permanent aperture to the Vel'thara resonance plane. Accord signed Day 244. See `maps/velthara-map.md`.

### Keeper Stations
| Station | Location | Reading | Status |
|---------|----------|---------|--------|
| Fenhollow | Marshes (G8) | 91% | Full relay hub. 8 Heartstones. |
| Aelindra | Sub-level 3 (N5) | 58% | Repaired. Map room. Vault. |
| Ashara | Beneath Vel'arath (G6 access) | Relay-reset | Fortified. Remote hard-seal. |
| Vel'arath (Station Four) | Old quarter (F7) | 91% | **COMPROMISED** — wall cracked during activation. |

### Cascade Status
**CASCADE ENDED.** All 14 nodes active. Anchor Point provides remote monitoring + boost.

### Barrow Sites (from Edra Voss's research)

| # | Site | Location | Status | Est. Remains | Notes |
|---|------|----------|--------|-------------|-------|
| 1 | Barrow of Keth-Varos | NE of Barrowfen, Thornfield Road | **🔒 SEALED** | ~300 | Entrance collapsed + Arcane Lock |
| 2 | Barrow of Vel Tharis | Coastal cliffs N of Aelindra | **🔒 SEALED** | ~400–500 | Sea cave collapsed + Arcane Lock |
| 3 | Ashenmere Catacombs | Beneath ruined temple, hills E of Greymoor | **🔒 SEALED** | ~200–300 | Heartstone discovered. Greymoor garrison posted. Aldric's quest resolved. |
| 4 | The Drowned Barrow | Fenhollow Marshes, near Three Sealed Ruins | **🔒 SEALED** | ~250 | Tidal access. Sealed Day 155. |
| 5 | Thornhallow Ossuary | Deep Thornveil Forest, 3 days N of Fenhollow | **🔒 SEALED** | ~350 | Preservation wards. Clay vessel undisturbed. Sealed Day 159. |
| 6 | The Salt Crypts | Coastal caves S of Salthollow | **🔒 SEALED** | ~200 | Salt-preserved. Natural halite deposit. Sealed Day 189. |
| 7 | Mirren's Rest | River island in the Mirren, upstream of Harrowgate | **🔒 SEALED** | ~400 | River resonance basin. Sealed Day 168. |

---

## COMPLETED ROUTES

### Eastern Outer Ward Ring (Session 5 — Chronicler/Storyteller)

```
⚒ Warden's Forge (crystal cut, rings enchanted)
  │
  ↓ 2 days
  │
⊗ Node 11 — Sethrak's Threshold ✔ (75%, RECONSTRUCTED, 14 wraiths freed)
  │
  ↓ 10 days south (via Greyveil → Greymoor → Harrowgate → Aelindra)
  │
🏛 Aelindra (recuperated, resupplied, allies visited)
  │
  ↓ half-day north
  │
⊗ Node 9 — Vel Tharis ✔ (78%, REPAIRED)
  │
  ↓ 2 days by ship (*Tern's Wing*)
  │
⊗ Node 10 — Cape Ashward ✔ (80%, REPAIRED)
  │
  ↓ 14 days (ship to Aelindra → overland via Salthollow → Harrowgate → Fenhollow)
  │
⌂ Dawnwatch Homestead, Fenhollow
```

### Western Arc (Session 5 — Wildcard)

```
⌂ Homestead → Greymoor (resupply) → Greypeak Pass → Vigil Stone Road
  │
  ↓ 7 days
  │
⚒ Warden's Forge (Node 5 crystal cut, all armor enchanted +1 AC)
  │
  ↓ 6 days (Shattered Stair → Tide Pools of Arn)
  │
⊗ Node 3 — Storm Pillar ✔ (62%, REPAIRED, behir nest cleared)
  │
  ↓ 2 days south
  │
⊗ Node 5 — Stormcape ✔ (74%, RECONSTRUCTED, Dead Zone clearing)
  │
  ↓ Stormcape coast exploration (65 days — 5 cliff-holds, alliance built)
  │
⊗ Node 2 — Drowned Forge ✔ (71%, REPAIRED, Ashfire Isle, Warden Drell honored)
  │
  ↓ Sail home via Tidemeet (homestead refit 25 days — all weapons enchanted)
  │
⊗ Node 4 — Tidewatcher ✔ (76%, INFUSED, Pip's flawless procedure)
  │
  ↓ Trimaran conversion at Driftwood Landing (14 days)
  │
⌂ Dawnwatch Homestead ★ PARTY IS HERE
```

**ALL 8 KNOWN OUTER WARD NODES ACTIVE — first time in 1,248 years.**

---

## TRADE OPERATION — Dawnwatch Fleet

| Detail | *Stormcape's Promise* | *Eilistraee's Grace* |
|--------|----------------------|---------------------|
| **Type** | Trimaran (28 ft, wind) | Trimaran (32 ft, hybrid wind + resonance) |
| **Captain** | Maren Voss | Lyriel Vel'sharen |
| **Crew** | Corr (deckhand) | Dren (engineer/deckhand) |
| **Route** | Triangle: Tidemeet → Veldmere → Stormcape (summer). Tidemeet ↔ Veldmere (winter). | Northern Express: Tidemeet → Aelindra → Salthollow → Tidemeet |
| **Revenue** | 100–150 gp/month | 120–180 gp/month (est.) |
| **Status** | Operational (Day 163, 1248 AS) | **Operational** (Day 173, 1249 AS). Also serves as expedition vessel. |
| **Home port** | Tidemeet | Tidemeet |
| **Trade Guild** | Thessaly's endorsement — 5–10% savings | Same (fleet-wide) |
| **Details** | See `maps/sea-routes.md` | See `maps/sea-routes.md` |

---

## STORMCAPE ALLIANCE

| Cliff-Hold | Tide-Speaker | Pop. | Notes |
|------------|-------------|------|-------|
| Arn's Hold | Tide-Speaker Arn | 26 | Pigeon network to homestead |
| Kell's Reach | Tide-Speaker Kell | 18 | Maren Voss's mother's home |
| Venn's Shelf | Tide-Speaker Lira | 14 | One eye. Reported Node 4 light. |
| Driftwood Landing | Tide-Speaker Venn, Old Marsh | 30 | Trimaran built here. Shipwright. |
| Saltspray | Grandmother Wynn, Asha | 8 | Asha (folk ward-keeper, Node 4). Smallest hold. |

---

## UNDERDARK ENTRANCES (Preliminary)

See `maps/underdark-map.md` for full details. Summary:

| # | Name | Location | Status | Confidence |
|---|------|----------|--------|------------|
| U1 | Undercroft (Duskport) | K12 | Confirmed — partially explored | High |
| U2 | Greymoor Deep Shafts (Shaft 9) | E5 | Confirmed — sealed door at 600 ft | High |
| U3 | Ashenmere Lower Vault | F4 | Confirmed — sealed, garrison posted | High |
| U4 | Greypeak Pass Caverns | E3 | Suspected | High |
| U5 | Thornveil Sinkholes | F6–G7 | Suspected — 2 sinkholes scouted | High |
| U6 | Three Sealed Ruins (Marshes) | G8 | **ACCESSED** — keeper station active (Node 7, 91%) | High |
| U7 | Warden's Forge Tunnels | D1 | Suspected — Vaelith confirms sealed | High |
| U8–U14 | Various (see underdark-map.md) | Various | Suspected/Rumor | Medium–Low |
| U15 | Aelindra Keeper Station | N5 | **ACCESSED** — 58% repaired | High |
| U16 | Sinkhole / Ashara Station | G6 | **ACCESSED + SEALED** — ward-locked, alarmed | High |
| U17 | Station Four (Vel'arath) | F7 | **ACCESSED** — 91%, **COMPROMISED** (wall cracked) | High |

---

## INTER-CITY ALLIANCE NETWORK

| City | Contact | Role |
|------|---------|------|
| Duskport | Magistrate Sera Voss | Civic inquiry lead |
| Duskport (counter-intel) | Serevane | Intelligence operations |
| Harrowgate | Alderman Kellan | Civic authority |
| Greymoor | Judge Aldara Kess | Warrants + civic reform |
| Greymoor (garrison) | Commander Cole | Ashenmere Heartstone guard |
| Salthollow | Councilwoman Brynn Ashworth | Civic authority |
| Fenhollow | Captain Jorik Brenn | Militia commander (reinstated) |
| Port Veldmere | Councilman Fell + Revka Sarn (Silver Chain) | Civic + maritime |
| Aelindra | Veyra Solund (interim Archivist-General) | Archive governance |
| Aelindra | Dalla (The Inkwell) | Safehouse contact |
| Maritime | Revka Sarn / Sable (Silver Chain) | Sea lane enforcement |
| Cape Ashward | Corran Ashward | Village headman |
| Stormcape | Arn, Kell, Lira, Venn, Wynn | 5 cliff-hold Tide-Speakers |
| Stormcape | Old Marsh (Driftwood Landing) | Master shipwright |
| Homestead | Tessa Marsh | Caretaker |
| Fenhollow | Sister Venn (Chapel of the Marsh Light) | Temple ally |
| Trade | Maren Voss (captain, *Stormcape's Promise*) | Merchant operation |
| Trade | Dalla Kern (Veldmere broker) | Standing herb order |
| Tidemeet | Soren Blackwater (warehouse manager) | Dawnwatch Trading Company |
| Deephollow (Underdark) | Elder Garrin | Svirfneblin trade (amethyst, mushrooms) |
| Crystalvein (Underdark) | Elder Brill | Svirfneblin trade (sapphires) |
| Ashport | Dalla Cray (potential recruit) | Southern port contact |
| Anchor Point | Vaelen + Warden Drell | Vel'thara embassy keepers |
| Vel'thara (beyond boundary) | The Speaker (Song Bridges harmony) | Extradimensional ambassador |

---

*Last updated: Day 260, 30th Frostmere, 1249 AS*
*Party location: Dawnwatch Homestead, Fenhollow. Anchor Point embassy active (Ashport).*
*Campaign status: ARC 1-4 COMPLETE. ARC 5 BEGUN (Vel'thara). ALL 14 NODES ACTIVE. 4 keeper stations + Anchor Point. Vel'thara embassy operational. Ashara captured. Party Level 4.*

---

## Resonance Lattice — Keeper Positions (Discovered Session 12)

> The Resonance Lattice is a pre-Gardener planetary communication network, separate from the 14 ward nodes. Built by the Tenders (~7,400+ years ago). 14 Keepers + 1 Listener (Hub Seven).

### Integrated Keepers

| Keeper | Location | Depth | Status | Notes |
|--------|----------|-------|--------|-------|
| **Vael (K1)** | Deep ocean, ~200 mi SE of Tidemeet | Ocean floor | ✅ Integrated (Planting #94) | Ocean-layer relay. Guardian at trench. |
| **Kael-vos (K2)** | Deep ocean trench | Trench rim | ✅ Integrated (Planting #95) | Foreign (Hub Three). Purified. Guardian at trench. |
| **K7** | Tidemeet coastal bedrock (I9) | 200 ft | ✅ Integrated (Planting #97) | Threshold anchor → Listener's Chamber. |

### Awake Keepers (Awaiting Seeds)

| Keeper | Location | Depth | Status |
|--------|----------|-------|--------|
| K3 | Eastern continental shelf | 400 ft | Awake. Pulse 44 sec. |
| K4 | Northern ocean floor | 2,200 ft | Awake. Pulse 46 sec. |
| K5 | Western ocean floor | 3,100 ft | Awake. Pulse 43 sec. |
| K6 | Southern shelf (near Kapuluan) | 600 ft | Awake. Pulse 41 sec. |
| K8 | Deep ocean, western approach | 4,800 ft | Awake. Pulse 47 sec. Heavy sediment. |
| K9 | Volcanic ridge, south | 1,800 ft | Awake. Pulse 40 sec. Geothermal. |
| K10 | Northern ice shelf | 900 ft | Awake. Pulse 45 sec. Cold-slowed. |
| K11 | Western continental shelf (Vel'mora) | 350 ft | Awake. Pulse 39 sec. Kah-sol proximity. |
| K12 | Deep trench, southwest | 6,000 ft | Awake. Pulse 48 sec. Deepest. |
| K13 | Mid-ocean ridge | 3,400 ft | Awake. Pulse 43 sec. |
| K14 | Southern archipelago deep | 2,600 ft | Awake. Pulse 42 sec. Near Puso ng Dagat. |

### The Listener (Hub Seven)

| Detail | Info |
|--------|------|
| **Location** | Deep ocean trench, ~400 mi SE of Tidemeet |
| **Depth** | 12,200 ft (200 ft below trench floor) |
| **Status** | AWAKE. Root Seat planted (Planting #96). 3,217-year quarantine ended. |
| **Access** | Threshold from K7 (Tidemeet, 200 ft depth). Walk in, walk out. |
| **Facilities** | Relay Chamber (300 ft sphere), Archive (Tender documents), Crucible (Tender-tier forge), Seed Vault (1 seed/7 days) |
